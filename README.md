...
One of the many great things about Chaster is its community features. Not only does Chaster have a built in chat function it also has many community intractable lock functions. You can create shared links for other users or gusts to add or remove time on your lock, another feature is verification photos. You can submit a photo of yourself locked with a 6 digit code that others can confirm or deny your code is correct to verify you are still locked. Another popular feature is the pillory. This is a board were people can submit polls for people to vote on tasks or people can vote to add more time for other users locks. Users can be added to the pillory for more time to be added either automatically if certain conditions are not met or by the keyholder/dom directly.

The chat function has a "ignore conversation" and a report message function but aside from that, their is little users can do to protect themselves. So even if you "ignore" a user depending on how your lock is setup they can still add time, vote for tasks and your verification photos. Yikes! So what little can users do? For shared links you can see who voted and if they added or removed time (unless the user is a guest). So yo could simply reverse the action of uses you do not want to interact with you but this could be taxing on the user and users can vote once every 24 hours with no way to change this at all. There is not even an option at all to  reset the link so its a different url, the other option would be to flat out disable this feature. 

As for verifications and pillory your options are even worse. Their is NO way at all to see who voted for what. So unless you disable the features, users who you do not want interacting with you can interact all they want in negative ways with no way for you know if they are or not doing it. To me at least, your only opting being to disabled these features is just not a real answer to the problem. Its a temporary fix at best and ignores the underlying issue.

This does not even account for shared locks, think of these as pre made locks that users can join virtually at anytime. Now these can be made private and require a password but this still is not a real answer to the problem. If the lock is public any user can join them at any time and their is nothing you can do to stop users you do not want joining them from joining them. You can end the lock once someone starts it but the user can just start up the lock again! A popular type of shared lock is a trap lock, users join a generally very short lock and hope the lock creator does not find out and trap them. Being forced to use a password to protect it completely ruins the whole concept.

It's not like adding a blocking feature is a new idea either. From what I can find, someone suggested adding a blocking feature on 1/25/22, thats over one year ago. The topic of not being able to stop users from interacting with certain element's of the lock came up many times before then. Even on their own feedback website their are two posts, one from 9 months ago and one from 5 months ago asking for a blocking function. 
...
So, despite all of the requests and the need for a blocking system, its not planned until Q4 2023. Yikes! There are some things I want to note here. 
...
This one strikes me as very strange. For starters, from my understand he is talking about the search function that can be found inside the messages section of the website. From my experience... it works just fine. Not fast, not perfect but just fine. The second thing is "I'm sure you don't want a blocking system with a laggy experience." Their is already an ignore user function, why cant a block user option be added right below it? I believe most of the time users would want to block someone they will probably have some kind of interaction with the user. Assuming their is a block function in messages and or on the users profile it would be easy to find the user based on a previous or current interaction to block them. And as stated before their is a search feature that does indeed work if you still can not find them. I just do not understand this logic at all.

I understand that adding in a proper blocking feature will take many parts because it will have to work across many parts of the platform. This was my suggestion:
...
If you are confused by what I am saying don't worry. I have no idea if this was a good or bad suggestion because his response was... none, none at all. He never messaged me back. 

My main point still stands. Their has been plenty of time to shift development into working on these features instead. Over a year in fact, but his current plan is still not prioritize this and put it after other features later this year. If he truly understands the importance of it why has nothing been done sooner? Why is it still not a top priority? Even one of the lead moderators has stated "Yup. I agree it's sorely needed. But not that it's the literal highest priority". It would seem no matter how much feedback or need their is "I'm sure I'm (Vince) better placed to know what to prioritize on the app I'm developing."

The Chaster team likes to point to the moderation team as the current solution to this but the moderation team is not the end all be all. Chaster is tightly integrated around Discord, they have a very active server, you can get DM logs on Discord about your locks and messages, and you can even have you Discord username displayed and updated automatically. THe problem is this means that a lot of things happen off platform from chaster, and screen shots can be faked. So, it is understandable that the moderation team can only do so much in a lot of cases. While reporting problematic users is still important, letting users have control over their own accounts and letting them control who and who can not interact with them is paramount. 

Chaster knows this is a problem, not only have they acknowledged it they even recently made a post warning users about it
...
And yet the biggest solution is only a promise to come later this year, despite the fact users have been asking for it for over a year.
...
This is because Chaster assumes you have some means to lock your keys up with, normally this is done with a lock box. This is all fine and dandy right? If you create a lock with your keys locked up surely this is some way to unlock yourself and get free in an Emergency right? Wrong! This function does not exist at all. In fact its been suggested! 
...
But this suggestion was denied, because it will be abused by people not serious about Chastity! I can understand if this suggestion is not perfect because it leaves the lock "frozen" but surely an emergency that ends the lock would be fine? Currently as it stands if you end or as Chaster puts it "Abandon" a lock it will NOT give you your lock code. Your only options are to rely on a hygiene opening as your emergency unlock, make an alt account and give it keyholder permissions to let you out or ask the Chaster moderation team. 

Surely you can select no option at all right? Wrong! In fact someone else suggested this and wouldn't you know it this suggest was also denied.

![The above suggestion](https://cdn.discordapp.com/attachments/833171605366243330/1121904989246332969/brave_gsVMC2TXxD.png)

This also infuriates me. This is not a "very edge case". Lots of users, I would argue probably most get a lock box *after* they get a cage or other Chasity device. Assuming that everyone has a method to lock their keys up is bonkers to me. If anything, wouldn't inexperienced users need the option to unlock at a moments notice __even more__ then an experienced user? This logic is absolutely bonkers to me. Coding wise the only thing that should be stopping this from being an option is removing a code requirement before starting a lock and making sure the program can expect no code when it would normally show the user a code. That's about it.

At the very least it is good that their is at least the option of going to the moderator team to ask for a code if need be, but why not having an emergency release system not standard is absolutely beyond me. Let alone a suggestion that gets denied. A friend of mine has left the platform over this feature alone. If chaster is serious about "the security of our users is indeed our top priority" which is a direct quote from Vince, why is this not a thing? However I will mention they are planning on adding an emergency unlock with a password which is fine enough I guess. Expect, as far as I can tell this has been in their back log for OVER 2 years and one the head mods of Chaster does not even think it should be an option at all.
...
"nor do I like the idea of any sort of emergency unlock" This absolutely infuriates me! Chaster is a public platform, so it is understandable that people with all levels of experience will be using this platform. The fact the do not want ANY kind of emergency unlock is just insane to me. I fully agree with LycraShortzUK here, not only should this already be a feature but the fact that a moderator from Chaster thinks it should not exist at all is quite frankly gross. This is ironic giving this is the mod who sings the praises of "Safe, Sane, Consensual." It is bold to assume all users have access to another breakable lock for emergency's when quite frankly not all users have access to a lock box as is. At least 99 other users felt this suggestion was worth an upvote despite this.

The last item I want to mention here is that the option to submit a code of your own is "planned". This can be used to submit photos of your keys or cage locked with a single use sealed lock with a random string on it. I would show a photo of this but I honestly can not find it again in the feedback site. If you do find it please let me know so I can update this section!
